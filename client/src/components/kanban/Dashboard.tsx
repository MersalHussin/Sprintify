...
// 2. تعريف الـ Props اللي الـ Dashboard مستنيها من بره
interface DashboardProps {
  onSelectBoard: (id: string | number, title: string) => void;
}
        ...
        <div className="max-w-6xl w-full mx-auto">
          ...
          <div className="flex justify-between items-center mb-10 border-b border-gray-200 pb-5">
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Workspaces</h1>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            {boards.map(board => (
              <div 
                key={board.id}
                onClick={() => onSelectBoard(board.id, board.title)}
            ...
            <div 
              onClick={handleAddBoard}
              className="h-36 bg-gray-200/50 hover:bg-gray-200/80 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center gap-2 text-gray-500 font-semibold cursor-pointer transition-all duration-300 hover:border-blue-400 hover:text-blue-600"
            >
              <FaPlus size={20} />
